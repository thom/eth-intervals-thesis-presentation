...
\note{ We implement several different variants of the \emph{Cache
    Stress Test}, each having different locality properties. The
  figure illustrates \emph{Best} and \emph{Worst Locality}:
...
\begin{frame}{Locality-Aware Intervals API}
  TODO
\end{frame}
...
The \lstinline!PlaceID! class encapsulates an integer ID and name. We
are using a class for the place ID instead of an integer due to type
safety.
...
\lstinline!Config! class to get a \lstinline!PlaceID!  object.
...
of the \lstinline!Places! class. Programmers can use it to get place
IDs, e.g. \lstinline!Config.places.getPlaceID(0)!.
