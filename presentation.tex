...
\begin{frame}{Alternative Designs}
  \begin{itemize}
  \item Other implementations provide each worker with mailbox in
    addition to work-stealing deque:
    \begin{itemize}
    \item FIFO queue of pointers to tasks having affinity for the
      worker
    \item Worker pushes work item onto both its deque and the tail of
      the mailbox of the worker the item has affinity for
    \item Worker first tries to get work from its mailbox before
      stealing
...
\note{
  ...
  \item Other locality-aware work-stealing schedulers provide each
    ...
    \begin{itemize}
    \item A mailbox is a FIFO queue of pointers to work items that
      have affinity for the worker.
    \item So when creating a work item, a worker will push it onto
      both the deque, as in normal work-stealing, and also onto the
      tail of the mailbox of the worker that the interval has affinity
      for.
    \item Now a worker will first try to obtain work from its mailbox
  ...
  \item However, we have decided to simplify our scheduler
    implementation by using a shared deque per \emph{Work-Stealing
      Place}.
  \item We believe that this would not impact scalability as long as
    the places are not too large. We could show that up to 8 workers
    there is no significant difference between using a separate deque
    for each worker or a shared deque per place.
  \end{itemize}
}
...
\note{
  ...
  \item In recent Java Virtual Machines, threads are implemented with
    ...
    between Java and native threads. When using the GNU C library on
    Linux, native threads are implemented with the Native POSIX
    Threads Library where each thread maps to a kernel scheduling
    entity.
  ...
  \item To bind the workers to a specific core, we wrote a small JNI
    library:
    \begin{itemize}
    \item With \lstinline!pthread_self()! we get the native thread ID.
...
\note{
  ...
  \item In the Java HotSpot VM, the NUMA-aware allocator has been
    ...
    \begin{itemize}
    \item The allocator relies on a hypothesis that a thread that
      allocates the object will be the most likely to use the object.
    \item To ensure the fastest access to the new object, the
      allocator places it in the memory region local to the allocating
      thread.
    \end{itemize}
...
\note{
  We present our performance evaluation of LASSI
}
...
\note{ 
  Like in the threaded version, we implement several different
  variants of the benchmark, each having different locality
  properties. The figure illustrates \emph{Best} and \emph{Worst
    Locality}:
  ...
  \begin{itemize}
  \item When using \emph{Ignorant Locality}, the intervals do not have
    any locality set.
  \item With \emph{Random Locality}, the locality of the intervals is
    set to a \emph{random} place.
  \end{itemize}
}
...
\note{
  ...
  \item When running the intervals implementations of the \emph{Cache
    ...
    about than 12\% faster compared to the other locality benchmarks.
  \item The \emph{best locality} benchmark has up to $1.5\times$ more
    L3 cache read hits and $3.1\times$ fewer L3 cache read misses than
...
\begin{frame}{Merge Sort}
  ...
  \item Uses divide-and-conquer to recursively sort \numprint{4194304}
    ...
    \end{itemize}
  \item Creates \numprint{8192} sorter intervals per worker, i.e. $8
    \times \numprint{8192}$
  ...
  \item Mergers merge two neighboring sorted arrays into one sorted
    array until all subarrays are merged into one single array
  \end{itemize}
\end{frame}
...
\note{
  ...
  \item Merger intervals merge two neighboring sorted arrays into one
    sorted array of doubled size until all subarrays are merged into
    one single array.
  \end{itemize}
}
...
\note{
  ...
  \item The \emph{best locality} implementation puts data sharing
    ...
    locality set for \emph{place 1}. Merger intervals copy the
    locality of the interval they get the left array to merge from,
    i.e. they are scheduled on the same place as their left
    predecessors in the tree hierarchy.
  \item When using \emph{Worst Locality}, the sorter intervals have
    ...
    other place as their left predecessor in the tree hierarchy. This
    means, if the left predecessor was scheduled on \emph{place 0},
    then the merger is scheduled on \emph{place 1} and vice versa.
...
\note{
  ...
  \item The difference between the number of cache read hits and
    misses is only marginal: The \emph{best locality} variant has just
    about 2\% more cache read hits and 7\% less cache read misses than
    the \emph{worst} and \emph{ignorant locality} variants:
    \begin{itemize}
...
\note{
  ...
  when they are done, the 4 additions can also be computed in
  parallel.
  ...
  quadrants until the base case of size $32 \times 32$ is
  reached. Then it multiplies the corresponding $32 \times 32$
  submatrices of $A$ and $B$ using sequential matrix multiplication to
  add them up.  
}

\begin{frame}{Block Matrix Multiplication: Quadrants}
  TODO
\end{frame}
...
\note{
  ...
  \item[Best Locality:] The \emph{best locality} benchmark runs all
    ...
    quadrants 0 and 3 in \emph{place 0} and the ones of the quadrants
    1 and 2 in \emph{place 1}. This way the places are able to share
    their local L3 cache in an efficient way.
...
\note{
  ...
  Table shows the execution times and the speedups over the sequential
  algorithm for the \emph{best locality} and \emph{worst locality}
  benchmark implementations. As expected, the implementation with
  \emph{best locality} is faster than the one for the \emph{worst
    locality}.

  The figure depicts the execution time of the \emph{worst locality}
  implementation normalized to that of the \emph{best locality}
  implementation. The \emph{best locality} implementation shows a
  speedup over the \emph{worst locality} of about $1.07\times$.

  The difference between the number of cache read hits and misses is
  little: The \emph{best locality} variant has just about 2\% more
  cache read hits and 6\% less cache read misses than the \emph{worst
    locality} variant. The main reason for this is the rather small
  benchmark size and limited level of data sharing between the
  intervals. Last level cache misses can have a significant impact on
  the runtime nevertheless as is shown by the speedup of $1.07\times$
  of the \emph{best locality} variant over the \emph{worst locality}.
}
...
\note{
  At the end we conclude and summarize our research, and give some
